...
/// @member paused Order status, true if currently paused (inactive). False otherwise.
/// @member deposit Unutilized deposit. This is the amount of the token specified for sale
///                 to the pool that is not currently being sold. This can be because the
///                 order is inactive currently or because previous pause operations resulted
///                 in some sales not being performed. This amount can be refunded on cancel
///                 or withdraw after order expiry. Extending the order may make use of these
///                 funds to extend as needed (to the furthest block interval at the existing
///                 sales rate).
///                 Min. = 0, Max. = (2**112) - 1
/// @member proceeds Captured proceeds. This is the amount of stored proceeds available for
///                  withdraw in addition to any proceeds currently available in the scaled
///                  proceeds system. These proceeds are accrued when an order is paused.
///                  These proceeds are remitted to the user when withdraw is called.
///                  Min. = 0, Max. = (2**112) - 1
...
/// @member orderStart is the block when this order was originally issued. Extensions,
///                    pausing and resuming the order do not change this number.
struct Order {
  bool token0To1;
  // --- Pause functionality ---
  bool paused;
